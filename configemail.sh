...
if [[ ! $REPLY =~ ^[Nn]$ ]]; then
    ...
    echo "Test de reconfiguration TeslaMate Mail - $(date)" | mail -s "Test Reconfig Email" -r "$SMTP_FROM" "$DEFAULT_EMAIL"
    
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✓ Email de test envoyé !${NC}"
        echo -e "${CYAN}Vérifiez votre boîte de réception (et vos spams).${NC}"
    else
