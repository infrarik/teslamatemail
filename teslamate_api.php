...
try {
    ...
    $sql = "
        SELECT 
            ...
            s.state,
            c.name,
            (SELECT version FROM updates WHERE car_id = c.id ORDER BY end_date DESC LIMIT 1) as version,
    ...
    if ($row) {
        $rawState = $row['state'];
        $isCharging = ($rawState !== 'asleep') && ($row['active_charging_sessions'] > 0 || (float)$row['charger_actual_power_kw'] > 0);
        
        $data = [
            "name"                 => $row['name'],
            "version"              => $row['version'] ?? null,
            "state"                => $isCharging ? "charging" : $rawState,
