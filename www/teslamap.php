...
// --- 3. EXTRACTION IDENTIFIANTS ET IP ---
...
try {
    ...
    if ($selected_car_id) {
        ...
        $sql_charge = "SELECT SUM(charge_energy_added) as kwh FROM charging_processes WHERE car_id = :car_id AND DATE(end_date AT TIME ZONE 'UTC' AT TIME ZONE 'Europe/Paris') = :date";
...
} catch (PDOException $e) { $error_message = "Erreur SQL : " . $e->getMessage(); }
?>
...
<head>
    ...
    <style>
        ...
        .input-field { width: 100%; padding: 10px; background: #333; border: 1px solid #444; color: white; border-radius: 4px; box-sizing: border-box; outline: none; }
        ...
        .kwh-val { color: #3b82f6; font-weight: bold; }
        ...
        .legend { position: absolute; bottom: 30px; right: 20px; background: rgba(0,0,0,0.8); padding: 12px; border-radius: 8px; border: 1px solid #444; z-index: 1000; font-size: 11px; color: white; }
...
<div id="sidebar">
    ...
    <div class="stats-bar">
        ...
        <div class="stats-grid">
            <span><?= $txt['distance'] ?> : <span class="km-val"><?= number_format($total_km, 1, ',', ' ') ?> km</span></span>
            <span><?= $txt['charged'] ?> : <span class="kwh-val"><?= number_format($total_kwh, 1, ',', ' ') ?> kWh</span></span>
        </div>
    </div>

    <div class="list-container">
        <?php if (empty($trajets)) echo "<p style='text-align:center; color:#666;'>".$txt['none']."</p>"; ?>
        <?php foreach ($trajets as $t): ?>
            <div class="trajet-card <?= $selected_drive_id == $t['id'] ? 'active' : '' ?>" onclick="location.href='?car_id=<?= $selected_car_id ?>&date=<?= $selected_date ?>&drive_id=<?= $t['id'] ?>'">
                <div class="trajet-header">
                    <span style="font-weight:bold;"><?= date('H:i', strtotime($t['start_date_local'])) ?></span>
                    <span class="km-val"><?= $t['km'] ?> km</span>
                </div>
                <div style="font-size:11px; color:#aaa;"><?= $txt['to'] ?> : <?= htmlspecialchars($t['end_point'] ?? $txt['unknown']) ?></div>
            </div>
        <?php endforeach; ?>
    </div>
</div>
...
<script>
    ...
    <?php if (!empty($positions)): ?>
        ...
        const departIcon = L.divIcon({
            ...
            html: '<div style="background:#22c55e; color:#fff; width:30px; height:30px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-weight:bold; border:3px solid #fff; box-shadow:0 2px 8px rgba(0,0,0,0.3);">D</div>',
            iconSize: [30, 30], iconAnchor: [15, 15]
        });
        
        const arriveeIcon = L.divIcon({
            className: 'custom-marker',
            html: '<div style="background:#dc2626; color:#fff; width:30px; height:30px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-weight:bold; border:3px solid #fff; box-shadow:0 2px 8px rgba(0,0,0,0.3);">A</div>',
            iconSize: [30, 30], iconAnchor: [15, 15]
        });
        
        L.marker(pts[0], {icon: departIcon}).addTo(map).bindPopup("<?= $txt['start'] ?>");
        L.marker(pts[pts.length - 1], {icon: arriveeIcon}).addTo(map).bindPopup("<?= $txt['end'] ?>");
    <?php endif; ?>
</script>
</body>
</html>
