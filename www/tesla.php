...
function save_setup_key($file, $keyToSave, $valueToSave) {
    ...
    $found = false;
    $keyToSave = strtolower($keyToSave); // On force la clé en minuscule dans le fichier
    
    foreach ($lines as $idx => $line) {
        // Regex pour trouver "language=" au début de la ligne, insensible à la casse
...
// 1. Chargement de la config actuelle
$config = read_setup($file);

// 2. Traitement de la requête AJAX (changement de langue via clic drapeau)
if (isset($_GET['save_lang'])) {
    $lang = strtoupper(trim($_GET['save_lang']));
    if ($lang === 'FR' || $lang === 'EN') {
        save_setup_key($file, 'language', $lang);
        // On répond en JSON pour confirmer au JavaScript que c'est fait
...
// 3. Si la clé "language" n'existe pas du tout, on la crée immédiatement (FR par défaut)
...
// --- RÉCUPÉRATION DES AUTRES INFOS (IP, DB...) ---
  ...
  <script>
    ...
    const i18n = {
      fr: {
        ...
        credits: "Crédits", referral: "Parrainage Tesla", charging: "EN CHARGE"
      },
      en: {
        ...
        credits: "Credits", referral: "Tesla Referral", charging: "CHARGING"
      }
    };
    ...
    async function setLanguage(lang) {
      ...
      try {
        const response = await fetch(`tesla.php?save_lang=${lang.toUpperCase()}&t=${Date.now()}`);
      ...
      app.innerHTML = `
        <div class="min-h-screen pb-20">
              ...
              <div class="flex flex-col items-center space-y-4">
                ...
                <div class="w-full flex items-center justify-between">
                  ...
                  <div class="flex-1 flex justify-center items-center gap-4">
                    <a href="teslamap.php" title="${t.tooltip_trips}" class="p-2 bg-gray-700 rounded-lg hover:bg-gray-600 transition-colors">
                      <svg class="w-8 h-8 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="2" d="M9 20l-5.447-2.724A2 2 0 013 15.488V5.13a2 2 0 011.106-1.789L9 1m0 19v-19m0 19l6-3m-6-16l6 3m0 0l5.447-2.724A2 2 0 0121 4.512v10.358a2 2 0 01-1.106 1.789L15 20m0-19v19"></path></svg>
                    </a>
                    <a href="teslanotif.php" title="${t.tooltip_notif}" class="p-2 bg-gray-700 rounded-lg hover:bg-gray-600 transition-colors">
                      <svg class="w-8 h-8 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path></svg>
                    </a>
                    <a href="teslacalcul.php" title="${t.tooltip_export}" class="p-2 bg-gray-700 rounded-lg hover:bg-gray-600 transition-colors">
                      <svg class="w-8 h-8 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="2" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg>
                    </a>
                    <a href="teslaconf.php" title="${t.tooltip_conf}" class="p-2 bg-gray-700 rounded-lg hover:bg-gray-600 transition-colors">
                  ...
                  <button onclick="fetchData()" class="p-2 bg-gray-700 rounded-full hover:bg-gray-600 transition-colors">
          ...
          <div class="px-4 pt-6 space-y-4 max-w-2xl mx-auto">
            ...
            <div class="bg-gradient-to-br ${isCharging ? 'from-green-900/40 to-gray-900 border-green-700/50' : 'from-green-900/20 to-gray-900 border-green-700/30'} rounded-3xl p-6 border shadow-2xl transition-colors duration-500">
