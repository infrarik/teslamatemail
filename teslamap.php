        ...
        if(isSingleDrive) {
            ...
            pauses.forEach(p => {
                if (p.is_charge) {
                    ...
                    L.marker([p.lat, p.lng], {
                        ...
                        `Durée : <b>${dureeStr}</b>` +
                        (p.elevation !== null ? `<br>⛰️ Altitude : <b>${p.elevation} m</b>` : '') +
                        (p.temp !== null ? `<br>🌡️ Temp. ext. : <b>${p.temp}°C</b>` : '')
                    );
                }
            });
        } else {
            pauses.forEach(p => {
                if (p.is_charge) {
                    ...
                    const altStr  = p.elevation !== null ? `<br>⛰️ Altitude : <b>${p.elevation} m</b>` : '';
                    const tempStr = p.temp !== null ? `<br>🌡️ Temp. ext. : <b>${p.temp}°C</b>` : '';
                    L.marker([p.lat, p.lng], {
                        icon: L.divIcon({className:'pause-marker', html:'P', iconAnchor:[10,10]})
                    }).addTo(map).bindPopup(`Pause de ${p.dur} min${altStr}${tempStr}`);
                }
            });
        }
