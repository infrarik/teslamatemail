...
// 2. Traitement de la requête AJAX
  ...
  <script>
    ...
    const i18n = {
      fr: {
        ...
        tooltip_conf: "Configuration", tooltip_dashcam: "Dashcam", state: "État", odometer: "Kilométrage",
        ...
        version: "Version", asleep: "EN VEILLE", parked: "STATIONNEMENT", online: "EN LIGNE", offline: "HORS LIGNE"
      },
      en: {
        loading: "Loading...", refresh: "Refresh", tooltip_trips: "Trips",
        tooltip_notif: "Notifications", tooltip_export: "Export / Calculator",
        tooltip_conf: "Settings", tooltip_dashcam: "Dashcam", state: "Status", odometer: "Odometer",
        ...
        version: "Version", asleep: "ASLEEP", parked: "PARKED", online: "ONLINE", offline: "OFFLINE"
      }
    };
    ...
    async function setLanguage(lang) {
      ...
      render();
      try {
        await fetch(`tesla.php?save_lang=${lang.toUpperCase()}&t=${Date.now()}`);
    ...
    function render() {
      ...
      let displayState;
      if (isAsleep) {
        displayState = t.asleep;
      } else if (isCharging) {
        displayState = t.charging;
      } else if (carData.state === 'parked') {
        displayState = t.parked;
      } else if (carData.state === 'online') {
        displayState = t.online;
      } else if (carData.state === 'offline') {
        displayState = t.offline;
      } else {
        displayState = (carData.state || '').toUpperCase();
      }
              ...
              <div class="flex flex-col items-center space-y-4 text-center">
                ...
                <div class="w-full flex items-center justify-between">
                  ...
                  <div class="flex-1 flex justify-center items-center gap-4">
                    <a href="tesladashcam.php" title="${t.tooltip_dashcam}" class="p-1 bg-gray-700 rounded-lg hover:bg-gray-600">
                      <img src="dashcam_logosmall.jpg" class="w-10 h-10 object-contain" alt="Dashcam">
                    </a>
            ...
            <div class="grid grid-cols-3 gap-4">
              <div class="bg-gray-800 rounded-2xl p-4 border border-gray-700 shadow-xl">
                ...
                <div class="flex items-center gap-2">
                  <div class="w-2 h-2 rounded-full ${isAsleep ? 'bg-gray-500' : isCharging ? 'bg-green-400 animate-pulse' : (carData.state === 'online' ? 'bg-green-500' : 'bg-gray-500')}"></div>
                  <span class="text-lg font-bold uppercase ${isCharging ? 'text-green-400' : isAsleep ? 'text-gray-400' : ''}">${displayState}</span>
                </div>
              </div>
              ...
              <div class="bg-gray-800 rounded-2xl p-4 border border-gray-700 shadow-xl text-right">
                <p class="text-xs text-gray-500 uppercase font-bold mb-1">${t.version}</p>
                <span class="text-lg font-bold">${carData.version || '—'}</span>
              </div>
            </div>
