...
- **Pourcentages appliqués aux prix** : ajout ou réduction en % du prix de base, visible dans tous les calculs, rapports et exports. Option d'affichage/masquage du pourcentage appliqué.
...
- **Détection automatique des mises à jour** : l'écran principal vérifie silencieusement si une nouvelle version est disponible sur GitHub. Un bandeau jaune s'affiche avec **trois options** : télécharger uniquement les fichiers dans `/tmp`, télécharger et installer automatiquement, ou ignorer la mise à jour.
...
- **Gestion avancée des prix** :
  - Prix du kWh par lieu (geofence)
  - Application de pourcentages (ajout ou réduction)
  - Affichage optionnel du pourcentage appliqué
  - Mémorisation automatique des prix et pourcentages
...
- Un bandeau jaune **🔄 Mise à jour disponible** s'affiche avec **trois options** :
  1. **Télécharger** : télécharge `files.zip` et `installweb.sh` dans `/tmp` sans les installer
  2. **Télécharger et installer** : télécharge ET exécute automatiquement `installweb.sh` pour une mise à jour complète
  3. **Ignorer** : masque le bandeau jusqu'à la prochaine version
- Processus totalement automatisé — aucune intervention manuelle nécessaire avec l'option "Télécharger et installer"
...
2. Choisissez parmi trois options :
   - **Télécharger** : télécharge uniquement les fichiers dans `/tmp` (installation manuelle ultérieure)
   - **Télécharger et installer** : mise à jour complète automatique (recommandé)
   - **Ignorer** : masque le bandeau jusqu'à la prochaine version
...
| `SHOW_PERCENTAGE` | `True` / `False` — affichage du % appliqué |
...
- **Percentage adjustments for pricing**: add or reduce the base price by %, visible in all calculations, reports and exports. Optional display/hide of applied percentage.
...
- **Automatic update detection**: the main screen silently checks whether a new version is available on GitHub. A yellow banner appears with **three options**: download only to `/tmp`, download and install automatically, or ignore the update.
...
- **Advanced price management**:
  - Per-location kWh pricing (geofence)
  - Percentage adjustments (add or reduce)
  - Optional display of applied percentage
  - Automatic save of prices and percentages
...
- A yellow **🔄 Update available** banner appears with **three options**:
  1. **Download**: downloads `files.zip` and `installweb.sh` to `/tmp` without installing
  2. **Download and install**: downloads AND automatically runs `installweb.sh` for a complete update
  3. **Ignore**: hides the banner until the next version
- Fully automated process — no manual intervention needed with the "Download and install" option
...
2. Choose from three options:
   - **Download**: downloads files to `/tmp` only (manual installation later)
   - **Download and install**: complete automatic update (recommended)
   - **Ignore**: hides the banner until the next version
...
| `SHOW_PERCENTAGE` | `True` / `False` — display applied % |
