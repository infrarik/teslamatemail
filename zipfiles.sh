...
PACKAGE_DIR="/tmp/teslamate_package"
DEST_ZIP="/tmp/files.zip"
...
WWW_FILES=(
    "index.php" "auth.php" "auth_check.php"
...
# Génération du numéro de version (format année.jour.mois.incrément)
DATE_VERSION="$(date '+%Y.%d.%m')"
VERSION_FILE="$SRC_WWW/cgi-bin/version"
if [ -f "$VERSION_FILE" ]; then
    CURRENT=$(cat "$VERSION_FILE")
    CURRENT_DATE=$(echo "$CURRENT" | cut -d'.' -f1-3)
    CURRENT_INC=$(echo "$CURRENT" | cut -d'.' -f4)
    ...
    echo "Pas de fichier version existant, création initiale."
    NEW_INC=1
fi
...
if [ -f "$DEST_ZIP" ]; then
    echo -e "${GREEN}✓ Archive créée avec succès dans /tmp/files.zip${NC}"
else
